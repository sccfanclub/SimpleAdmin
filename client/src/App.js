...
class App extends Component {
    ...
    itemAdd({newProject, newName, newUrl}) {
        return (
            <div class="item_add">
                ...
                <input type="text" id="project" placeholder="please input project"
                ...
                <input type="text" id="url" placeholder="please input url"
    ...
    itemList({items}) {
        ...
        items.forEach(item => {
            ...
            projects[item.project].push({url: item.link.url, name: item.link.name});
        });
        return (
            <div>
                {Object.keys(projects).map((projectName, index) => (
                    <div id="item_list" key={index}>
                        <h3 style={{color: 'blue'}}>{projectName}:</h3>
                        {projects[projectName].map((item, index) => (
                            <div key={index}><a href={item.url}>{item.name}</a></div>))}
                    </div>
                ))}
